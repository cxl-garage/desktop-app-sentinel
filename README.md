...
2. After cloning and going into repo, run ``` npm install ``` in terminal to download these dependencies.
3. To start app, run ```npm start```

## If ```npm install``` displays errors
...
- preload.d.ts: declares window as global variable, which communicates with *ipcRenderer*
