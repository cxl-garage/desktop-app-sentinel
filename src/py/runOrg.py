...
#gets organization from user input
user_org = input()
user_org = json.loads(user_org)
directory = os.path.dirname(os.path.realpath(__file__))
#configs log file
logging.basicConfig(filename = os.getcwd()+"\src\py\logfile.csv",
                    encoding='utf-8',
                    filemode='w',
...
def main(opt,container=None):
    ...
    try:
        ...
        if GPU_num == 0:
            logger.info('Starting tensorflow container optimized for CPUs')
        else:
            logger.info('GPU support does not yet exist')
    except Exception as e:
       # logger.info('Error: '+ e)
        logger.error('Error: ' + e)
        GPU_num = 'Unknown'
    logger.info(f"CPUs Available: {os.cpu_count()}, GPUs Available: {GPU_num}")
    ...
    try:
        ...
        logger.info('Container created successfully')
            ...
            if platform.system() == 'Windows':
                logger.info('Adding auth key to Windows')
                query = f'docker login -u _json_key --password-stdin https://us-west2-docker.pkg.dev < {opt.key}'
            else:
                logger.info('Adding auth key to Linux/Mac')
                query = f'cat {opt.key} | docker login -u _json_key --password-stdin https://us-west2-docker.pkg.dev'
            logger.info(query)
            os.system(query)
            while True:
                logger.info(f'Downloading Image from Google Cloud Platform with {opt.key} credentials')
                container = client.containers.run(container_name,detach=True, name=f'sentinel',ports={8501:8501},cpu_count=num_workers,mem_limit='5g')


    available_algs = utils.check_available_algs(container).split(',')
    dictionary = [{
        "models": available_algs
    }]

    # Serializing json
    json_object = json.dumps(dictionary, indent=4)

    # Writing models available to organization to Models.json
    with open(os.getcwd()+"\src\py\Models.json", "w") as outfile:
        outfile.write(json_object)

    logger.info('Available models found')

    print('success', available_algs)
    # available_algs = utils.check_available_algs(container).split(',')
    #logger.info('Available models found')
    return
