...
#print(user_input['org'], user_input['model'])
...
logging.basicConfig(filename = os.getcwd()+"\src\py\logfile.txt",
                    encoding='utf-8',
                    filemode='w')
...
def process(filename):
    ...
    if len(glob.glob(f"{output}/**/{os.path.basename(filename)}")) != 0:
        ...
        try:
            ...
            logger.error('detections error' + e)
    ...
    try:
        ...
        if GPU_num == 0:
           ...
           # print('Starting tensorflow container optimized for CPUs')
            ...
            logger.error('GPU support does not yet exist')
...
def run():
    ...
    if user_input['Confidence threshold']:
      threshold=user_input['Confidence threshold']
    else:
      threshold=40
    input=user_input["Import from"]
    output=user_input["Save to"]
    ...
    if not inputExist or not outputExist:
      ...
      logger.error("Directories don't exist")
      return
